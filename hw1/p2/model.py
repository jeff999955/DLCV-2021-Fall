...
# Implementation thanks to https://github.com/say4n/pytorch-segnet/
DEBUG = False
...
class DeepLabv3_ResNet101(nn.Module):
    def __init__(self, num_classes = 7):
        ...
        self.name = "DeepLabv3_ResNet101"
        self.num_classes = num_classes
        self.model = models.segmentation.deeplabv3_resnet101(num_classes=num_classes)

    def forward(self, x):
        x = self.model(x)['out']
        return x
    def load_pretrained(self):
        msd = torch.utils.model_zoo.load_url('https://download.pytorch.org/models/deeplabv3_resnet101_coco-586e9e4e.pth', progress = True)
        td = []
        for k in msd:
        ...
        for k in td:
            msd.pop(k)
        msd["classifier.4.weight"] = torch.zeros((self.num_classes, 256, 1, 1))
        msd["classifier.4.bias"] = torch.zeros(self.num_classes)
...
if __name__ == '__main__':
    # i = torch.Tensor(8, 3, 512, 512).cuda()
    m = DeepLabv3_ResNet101(7)
    print(m)
    # od = m(i)
    # print(od.shape)
